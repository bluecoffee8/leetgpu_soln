...
@triton.jit
def softmax_fused_kernel(input, output, N, BLOCK_SIZE: tl.constexpr):
    # Single CTA handles all N elements: one pass for max, one for exp+normalize
    offsets = tl.arange(0, BLOCK_SIZE)
    mask = offsets < N
    x = tl.load(input + offsets, mask=mask, other=-float("inf"))
    m = tl.max(x)
    z = tl.where(mask, tl.exp(x - m), 0.0)
    s = tl.sum(z)
    tl.store(output + offsets, z / s, mask=mask)
...
def solve(input: torch.Tensor, output: torch.Tensor, N: int):
    ...
    if BLOCK_SIZE <= 131072:
        softmax_fused_kernel[(1,)](input, output, N, BLOCK_SIZE=BLOCK_SIZE)
    else:
        BLOCK_SIZE = 2048
        # Initialize to -inf so atomic_max works correctly for all-negative inputs
        maxi = torch.full((1,), float('-inf'), device=input.device)
        div = torch.zeros(1, device=input.device)
