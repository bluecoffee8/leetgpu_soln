...
def softmax_attn(q_ptr, k_ptr, v_ptr, o_ptr, M, N, d, 
    ...
    softmax_max = tl.full([BLOCK_SIZE_M], float("-inf"), dtype=tl.float32)
    ...
    for i in range(0, N, BLOCK_SIZE_N):
        ...
        attn_mask = mM[:, None] & mN[None, :] # only for border cases, 
    ...
    tl.store(o_ptr + oO, acc.to(tl.float16), mask=mO)
...
def solve(
    ...
    softmax_attn[grid](Q, K, V, output, M, N, d,
                        stride_q0, stride_q1,
                        stride_k0, stride_k1,
                        stride_v0, stride_v1,
                        stride_o0, stride_o1,
