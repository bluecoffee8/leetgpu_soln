...
__global__ void vectorCopy(const float* a,
                          float* b,
                          int n)
{
...
int main(int argc, char** argv)
{
    ...
    // Warm-up run
    vectorCopy<<<blocks, threadsPerBlock>>>(d_a, d_b, N);
    CUDA_CHECK(cudaDeviceSynchronize());

    // Benchmark
    CUDA_CHECK(cudaEventRecord(start));

    vectorAdd<<<blocks, threadsPerBlock>>>(d_a, d_b, N);
    ...
    for (int i = 0; i < N; ++i) {
        float expected = h_a[i];

        if (std::fabs(h_b[i] - expected) > 1e-5f) {
            ok = false;
    ...
    double bandwidthGBs =
        (2.0 * bytes) / (seconds * 1e9); // read A + read B + write C
