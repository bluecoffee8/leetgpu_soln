...
__global__ __launch_bounds__(64) void matmul_vectorized(float* A, float* B, float* C, int M, int N, int K) {
    ...
    __shared__ float As[BLOCK_SIZE][BLOCK_SIZE+4], Bs[BLOCK_SIZE][BLOCK_SIZE+4]; 
    ...
    for (int n = 0; n < N; n += BLOCK_SIZE) {
        ...
        for (int i = 0; i < BLOCK_SIZE; i += a_stride) {
            ...
            As[a_n][a_m + i] = ldg_a_reg[ldg_index];
            As[a_n + 1][a_m + i] = ldg_a_reg[ldg_index + 1];
            As[a_n + 2][a_m + i] = ldg_a_reg[ldg_index + 2];
            As[a_n + 3][a_m + i] = ldg_a_reg[ldg_index + 3];
        }
        #pragma unroll
        for (int i = 0; i < BLOCK_SIZE; i += b_stride) {
            FETCH_FLOAT4(Bs[b_n + i][b_k]) = FETCH_FLOAT4(B[OFFSET(b_n + i, b_k, K)]);
        }
        __syncthreads(); 
        A += BLOCK_SIZE;
        B += BLOCK_SIZE * K; 
        ...
        for (int i = 0; i < BLOCK_SIZE; i++) {
            if (n + i < N) {
                #pragma unroll 
                for (int j = 0; j < T; j += 4) {
                    FETCH_FLOAT4(a_frag[j]) = FETCH_FLOAT4(As[i][tm + j]);
                }
                #pragma unroll
                for (int l = 0; l < T; l += 4) {
                    FETCH_FLOAT4(b_frag[l]) = FETCH_FLOAT4(Bs[i][tk + l]);
                }
