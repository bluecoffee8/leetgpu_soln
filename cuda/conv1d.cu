...
// Maps a logical s_input index to a physical smem slot by inserting one padding
// element every 32 logical positions. This turns the stride-4 access pattern
// (where 32 warp threads hit only 8 distinct banks) into a full 32-bank spread.
//
// Without padding: s_input[tid*4 + offset], bank = (tid*4 + offset)%32.
//   tid=0,8,16,24 all land on bank (offset%32) → 4-way conflict.
// With padding: bank = smem_pad(tid*4 + offset) % 32 = distinct for all 32 threads.
__device__ __forceinline__ int smem_pad(int i) { return i + (i >> 5); }

__global__ void conv1d_padded(const float* __restrict__ input, float* __restrict__ output,
                              int input_size, int kernel_size, int output_size) {
    extern __shared__ float s_input[];
    ...
    for (int t = tid; t < tile_size; t += bdim) {
        int global_idx = block_start + t;
        s_input[smem_pad(t)] = (global_idx < input_size) ? input[global_idx] : 0.0f;
    }
    ...
    if (global_idx < output_size) {
        ...
        for (int j = 0; j < kernel_size; j++) {
            ...
            #pragma unroll
            for (int k = 0; k < ITEMS_PER_THREAD; k++) {
                sum[k] += s_input[smem_pad(thread_offset + k + j)] * k_val;
            }
        }
...
extern "C" void solve(const float* input, const float* kernel, float* output, int input_size,
    ...
    int tile_size = threads * ITEMS_PER_THREAD + kernel_size - 1;
    // smem_pad(i) = i + i/32, so max physical index = (tile_size-1) + (tile_size-1)/32.
    // Allocate tile_size + tile_size/32 + 1 slots to cover all padded indices.
    int shared_mem_size = (tile_size + tile_size / 32 + 1) * sizeof(float);

    conv1d_padded<<<blocks, threads, shared_mem_size>>>(input, output, input_size, kernel_size, output_size);
